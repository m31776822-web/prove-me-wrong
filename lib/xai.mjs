...
const PROVIDERS = {
  deepseek: {
    ...
    refused: "That DeepSeek key was refused. Check it at platform.deepseek.com.",
    extra: { thinking: { type: "disabled" } },
  },
  xai: {
    url: "https://api.x.ai/v1/chat/completions",
    model: "grok-4.5",
    refused: "That API key was refused. Check it at console.x.ai.",
    extra: {},
  },
};
...
function detectProvider(key) {
  return String(key).startsWith("xai-") ? "xai" : "deepseek";
}
...
function failDetail(status, provider, body) {
  const spec = PROVIDERS[provider];
  if (status === 401 || status === 403) return spec.refused;
  if (status === 402) {
    return provider === "deepseek"
      ? "DeepSeek says this key is out of credit."
      : "The table is out of credit. Try again later.";
  }
  if (status === 429) return "Easy. Let the last round land.";
  if (body?.error?.type === "authentication_error") return spec.refused;
  const detail = typeof body?.error?.message === "string" ? body.error.message.replace(/\s+/g, " ").trim() : "";
  if (detail && !/bearer|sk-|xai-/i.test(detail)) {
    return `The table went quiet (${status}): ${detail.slice(0, 140)}`;
  }
  return `The table went quiet (${status}). Try again.`;
}

async function complete(apiKey, args) {
  const key = String(apiKey ?? "").trim();
  if (!key) {
    return { ok: false, error: "Add a DeepSeek API key to sit down. Get one at platform.deepseek.com." };
  }
  ...
  const attempts =
    provider === "deepseek"
      ? [
          { model: "deepseek-flash", extra: spec.extra },
          { model: "deepseek-flash", extra: {} },
          { model: "deepseek-chat", extra: {} },
        ]
      : [{ model: spec.model, extra: spec.extra || {} }];

  let lastStatus = 0;
  let lastBody = null;

  for (const attempt of attempts) {
    const jsonModes = args.json ? [true, false] : [false];
    for (const json of jsonModes) {
      let res;
      try {
        ...
        return { ok: false, error: "Couldn’t reach the table. Try again." };
      }
      let body;
      try {
        body = await res.json();
      ...
      lastBody = body;
      if (lastStatus === 401 || lastStatus === 403 || lastStatus === 402 || lastStatus === 429) {
        return { ok: false, error: failDetail(lastStatus, provider, lastBody) };
      }
      if (json && (lastStatus === 400 || lastStatus === 422)) continue;
      break;
    }
    if (lastStatus !== 400 && lastStatus !== 404 && lastStatus !== 422) break;
  }
