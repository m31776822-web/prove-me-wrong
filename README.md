...
[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https://github.com/m31776822-web/prove-me-wrong&env=XAI_API_KEY&envDescription=xAI%20API%20key%20from%20console.x.ai)

## Sit down in 30 seconds

You need [Node 18+](https://nodejs.org) and an [xAI API key](https://console.x.ai).
...
No `npm install`. Zero dependencies. One file serves the table and talks to xAI.

This is **not a static site**. Don’t open `public/index.html` in a browser, and don’t use GitHub Pages — the table needs a server so the browser never talks to xAI directly.
...
4. Paste a key if the environment doesn’t have `XAI_API_KEY`.

### Vercel

Click **Deploy with Vercel**, add `XAI_API_KEY`, sit down on the public URL.
